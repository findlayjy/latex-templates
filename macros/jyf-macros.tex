%%%%%% GENERAL %%%%%%
\usepackage{etoolbox}
% Used for Brian's denotation brackets macro, and my semantic predicate macro (not the latter any more)

%% Symbols
% Nice tick and cross (requires pifont package)
...
\newcommand{\partof}{\nlc{part\hyph of}}
\newcommand{\beans}{\nlc{\null^*\idprd{bean}}}
\newcommand{\ktb}{\nlc{kick\hyph the\hyph bucket}}

%% Subscript command for general use
\newcommand{\sub}[1]{\ensuremath{_{#1}}}
%% Susbcript command for formatting in text mode
...
% asterisks to indicate a break
\newcommand{\starskip}{\smallskip\hfill*\blank*\blank*\hfill\smallskip}
...
% \obl and \objth take an optional argument: the subscript (defaults to θ if nothing given)
...
\newcommand{\source}[1]{\signed{#1}}
\newcommand{\urlsource}[1]{{\source{\url{#1}}}}
...
\newcommand{\csn}[2]{%
  ...
  \begin{tabular}[c]{c}
    \protect{#1}\\
    \scalebox{0.9}{\ensuremath{\begin{array}{c}#2\end{array}}}
  \end{tabular}%
}

\newcommand{\opc}[2]{ % off-path constraints: a top-aligned version of \csn
  \setlength\tabcolsep{1pt}
  \begin{tabular}[t]{c}
    \protect{#1}\\
    \scalebox{0.9}{\ensuremath{\begin{array}{c}#2\end{array}}}
...
\newcommand{\denotes}[1]{\ensuremath{\llbracket \textnormal{#1} \rrbracket}} % Requires stmaryrd package
% Brian Buccola's neat solution that suppresses whitespace when optional arguments are not present.
\newcommand*\ldenotes[2][]{%
  \ensuremath{%
    \llbracket
    \ifblank{#2}{\dots}{#2}
    \rrbracket
...
\newcommand{\translates}[1]{\translation{#1}}
...
% Giving PRED values
\newcommand{\uppred}[1]{\ensuremath{\up{pred} = \textnormal{`#1'}}}
