...
~jyf-macros.tex~ shouldn't be spelled out tho -- it's too long. That can be copied by the =new-latex= commands? -- No, keep it in this repo and allow =latexmkrc= to find it there.

* Settings
Gathering all the below settings in one place:
...
#+end_src
*** TODO Switch to expex for examle numbering?
...
<<bib-settings>>
<<hyperlink-settings>>
% \usepackage[hyphens]{url} % hyperref loads this anyway, but without the hyphens option, which helps with linebreaking
